...
def sample_sharegpt_requests(
    ...
    for i in range(len(dataset)):
        ...
        if prompt_len < 4 or (fixed_output_len is None and output_len < 4):
            ...
            continue
        if prompt_len > 65536 or prompt_len + output_len > 65536:
...
def main(args: argparse.Namespace):
    ...
    data = sorted(list([i.output_tokens for i in outputs]))
    sample_indices = np.arange(len(data))  # 0, 1, 2, ..., 999
    plt.figure(figsize=(10, 5))
    plt.scatter(sample_indices, data, alpha=0.5, s=10, color="blue")
    data = sorted([i[1] for i in input_requests])
    plt.scatter(sample_indices, data, alpha=0.5, s=10, color="red")

    plt.title("Scatter Plot of output_tokens")
    plt.xlabel("Sample Index")
    plt.ylabel("output_tokens")
    plt.savefig(f"%s_%s_con_%s_out_%s_%d.png" % (args.dataset_name,
                                                       args.num_prompts,
